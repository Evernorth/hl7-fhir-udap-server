...
module.exports.tieredTokenClientHandler = async (tokenRequestBody, internalIdpData) => {
	...
	console.log('Validating the inbound Okta request...')
	...
	const validatedRequestData = oktaPlatform.validateTieredOAuthRequest(internalIdpData, inboundRequestData)

	if(validatedRequestData) {
		try {
			console.log("Inbound Okta request verified! Making UDAP token request to the real endpoint...")
