...
module.exports.validateTieredOAuthRequest = (internalIdpData, inboundRequestData) => {
    ...
    try {
        ...
        console.log('A valid JWT was not provided from Okta...')
        console.log(error)
        return false
    }
}
...
module.exports.updateClientApp = async (clientId, ssVerifiedJwtObject, verifiedJwtJwks, apiClient) => {
    ...
    const policyDetails = await getAuthzPolicyDetails(process.env.OKTA_RESOURCE_SERVER_ID, clientId, apiClient)

    if(policyDetails) {
        console.log('An existing authorization policy was found. Uppdating...')
        await updateAuthzPolicy(process.env.OKTA_RESOURCE_SERVER_ID, policyDetails.policyId, policyDetails.policyRuleId, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
    }
    else {
        console.log('No authorization policy was found for this application. Creating...')
        await createAuthzPolicy(process.env.OKTA_RESOURCE_SERVER_ID, clientId, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
    }
...
module.exports.createClientApp = async (ssVerifiedJwtObject, verifiedJwtJwks, apiClient) => {
    ...
    try {
        ...
        const policyDetails = await createAuthzPolicy(process.env.OKTA_RESOURCE_SERVER_ID, createResponse.credentials.oauthClient.client_id, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
...
module.exports.deleteClientApp = async (clientId, apiClient) => {
    try {
        ...
        if(policyDetails) {
            ...
            await apiClient.authorizationServerApi.deleteAuthorizationServerPolicy({policyId: policyDetails.policyId, authServerId: process.env.OKTA_RESOURCE_SERVER_ID})
        }
