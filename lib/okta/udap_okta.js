...
const createAuthzPolicy = async (authorizationServerId, clientId, grantTypes, scopes, apiClient) => {
    ...
    policyModel.conditions.clients.include = [clientId]
...
//TODO: Error handling.
module.exports.updateClientApp = async (clientId, resourceServerId, ssVerifiedJwtObject, verifiedJwtJwks, apiClient) => {
    //GET
    //UPDATE
    //PUT
    ...
    const policyDetails = await getAuthzPolicyDetails(resourceServerId, clientId, apiClient)

    if(policyDetails) {
        console.log('An existing authorization policy was found. Uppdating...')
        await updateAuthzPolicy(resourceServerId, policyDetails.policyId, policyDetails.policyRuleId, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
    }
    else {
        console.log('No authorization policy was found for this application. Creating...')
        await createAuthzPolicy(resourceServerId, clientId, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
    }
...
module.exports.createClientApp = async (ssVerifiedJwtObject, resourceServerId, verifiedJwtJwks, apiClient) => {
    ...
    try {
        ...
        const policyDetails = await createAuthzPolicy(resourceServerId, createResponse.credentials.oauthClient.client_id, ssVerifiedJwtObject.body.grant_types, ssVerifiedJwtObject.body.scope, apiClient)
...
module.exports.deleteClientApp = async (clientId, resourceServerId, apiClient) => {
    try {

        const policyDetails = await getAuthzPolicyDetails(resourceServerId, clientId, apiClient)
        if(policyDetails) {
            console.log('An authorization policy was found for this application. Cleaning up...')
            await apiClient.authorizationServerApi.deleteAuthorizationServerPolicy({policyId: policyDetails.policyId, authServerId: resourceServerId})
        }
